...
DROP TABLE IF EXISTS `tech_expertise`;
...
CREATE TABLE users (
  ...
  totp_backup_code VARCHAR(255) DEFAULT NULL,
...
CREATE TABLE ticket_internal_notes (
  ...
  author_id    INT UNSIGNED  NOT NULL,
  body         LONGTEXT      NOT NULL,
  note         LONGTEXT      AS (body) STORED COMMENT 'alias for body',
  created_at   DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at   DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (ticket_id) REFERENCES tickets(id) ON DELETE CASCADE,
  FOREIGN KEY (author_id) REFERENCES users(id)  ON DELETE CASCADE,
...
-- ──────────────────────────────────────────────────────────────
-- CLIENTS (full production schema with CBS/integration columns)
-- ──────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `clients` (
  ...
  `district`            VARCHAR(100)  DEFAULT NULL,
  `local_govt`          VARCHAR(100)  DEFAULT NULL,
  ...
  `product`             VARCHAR(100)  DEFAULT NULL,
  `cbs_use`             VARCHAR(100)  DEFAULT NULL,
  `integration`         VARCHAR(255)  DEFAULT NULL,
  `integration_charge`  DECIMAL(10,2) DEFAULT NULL,
  `agreement_date`      DATE          DEFAULT NULL,
  `installation_date`   DATE          DEFAULT NULL,
  `expiry_month`        DATE          DEFAULT NULL,
  `num_branches`        INT           NOT NULL DEFAULT 0,
  `head_office_amc`     DECIMAL(10,2) DEFAULT NULL,
  `branch_office_amc`   DECIMAL(10,2) DEFAULT NULL,
  `cloud_charge_ho`     DECIMAL(10,2) DEFAULT NULL,
  `cloud_charge_branch` DECIMAL(10,2) DEFAULT NULL,
  `cloud_gb`            DECIMAL(10,2) DEFAULT NULL,
  `notes`               TEXT          DEFAULT NULL,
...
-- ══════════════════════════════════════════════════════════════
--  Import complete! Tables: 55+
--  Admin URL : /admin/
--  Login     : ankurinfotech8@gmail.com / Admin@12345
--  ⚠  Login gare-pachi TURANT password change garnus!
-- ══════════════════════════════════════════════════════════════
