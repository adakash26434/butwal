...
/* ── Split layout: main content + sticky side panel ─────────── */
.af-split {
  display: grid;
...
(function(){
  ...
  // List tab: always allow client-side switch (no reload)
  tabs[0].addEventListener('click',function(e){
    e.preventDefault();
    listEl.style.display='';formEl.style.display='none';
    tabs[0].classList.add('active');tabs[1].classList.remove('active');
  });
  // New/Form tab: client-side switch only when not in edit mode
  if(!isEdit){
